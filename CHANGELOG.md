...
### Fixed
- `TortoiseCanvas` rebuilt every committed element's `Path` on every display frame during playback (~500 elements × 120 Hz ≈ 60k Path constructions per second), so programs with a few hundred commands visibly stuttered. The canvas is now two stacked layers: committed output renders in a static `Canvas` that re-renders only when a command commits (at most ~20×/sec even at speed 10, plus `step()` / `seek(to:)` / `clear`), while only the in-progress stroke/arc and the tortoise sprite draw at display refresh rate ([#35](https://github.com/temoki/TortoiseGraphics2/issues/35))
...
### Added
- `TortoisePlayer` (TortoiseUI) — playback control for `TortoiseCanvas`: `isPaused` (pauses the underlying `TimelineView` schedule, so a paused canvas stops redrawing entirely), `step()` (advance one command while paused), `seek(to:)` (jump forward or backward to any command index), and `speedOverride` (viewer-side speed that takes precedence over the stream's `.speed()` commands without rewinding playback). `currentCommandIndex` and `isFinished` are observable for UI such as highlighting the currently executing command. Attach with the new `TortoiseCanvas(_:player:)` initializer; all existing initializers are unchanged ([#23](https://github.com/temoki/TortoiseGraphics2/issues/23))
...
### Changed
- CI: pull requests now run CI regardless of their base branch (so stacked PRs get feedback during review), and codecov statuses are configured via `codecov.yml` to tolerate SwiftUI view-internal lines that tests cannot execute (project threshold 1%, patch informational)
