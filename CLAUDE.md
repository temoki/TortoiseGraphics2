...
- `TortoiseUITests/DrawingScenarioCanvasTests` (macOS-only, `#if os(macOS)`) renders `TortoiseCanvas` via `ImageRenderer` (`scale = 2`, instant mode forced by prepending `speed = 0`) and compares PNGs in `Tests/TortoiseUITests/__Snapshots__/` with `.canvasGolden` (`precision: 0.995, perceptualPrecision: 0.98`, defined in `CanvasGoldenSnapshotting.swift`) to absorb OS-level antialiasing drift. `perceptualPrecision` requires **swift-snapshot-testing 1.19.6 or newer**, which is why `Package.swift` floors the dependency there rather than at the version SwiftPM would otherwise pick: through 1.19.5 the perceptual path passed a bare `CGRect` to `CIAreaAverage`, which Core Image on macOS 27 rejected with an uncaught exception that killed the whole test process (#49, fixed upstream in pointfreeco/swift-snapshot-testing#1120). That path is only reached when bytes differ, so a byte-identical run never exercises it — don't read a green local suite as proof the floor can be lowered.
