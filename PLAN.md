...
設計原則: **論理座標系(ワールド)とビュー座標系(ビューポート)を分離**し、画面回転・ウィンドウリサイズを「絵(コマンド列)の問題」にしない。

- タートルは無限の論理座標空間(中心原点・y軸上向き)に住み、コマンド列はすべて論理座標で記録する。絵はサイズを持たないベクターデータ
- ビューは論理空間への覗き窓(ビューポート)。回転・リサイズ時はSwiftUI `Canvas` + `onGeometryChange` で新サイズに再レンダリングするだけで、ベクターなので劣化しない(旧版の即時ラスタライズ方式では不可能だった)
- タートルAPIはビューサイズに依存させない。キャンバスサイズを参照するAPIは論理キャンバスサイズを返し、同じプログラムがどのデバイス・向きでも同じ作品を生む(決定性の確保)

ビューポートの合わせ方は `TortoiseCanvasView` のオプションとして提供する。

| モード | 動作 | 主な用途 |
|---|---|---|
| scale to fit(デフォルト) | 作品の論理サイズ(例: 400×400)を、レターボックス付きでビューに収める。SVGの `viewBox` と完全に対応 | ブロックエディタ・教育用途 |
| 1:1・原点中央 | ウィンドウが広がると見える範囲が広がる | 探索的なお絵かき |
| 描画範囲に自動フィット | 作品のバウンディングボックスに合わせる | ギャラリー表示 |

### 4.4 ブロックエディタアプリ(TortoiseBlocks)
